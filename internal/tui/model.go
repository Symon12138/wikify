...
// matching the interactive TUI model interaction model.
package tui
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case PageRetryingMsg:
		if i, ok := m.slugIndex[msg.Slug]; ok {
			// Auto-retry never sent Failed first — never blindly decrement failCount.
...
func (m Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "down", "j":
		...
		case "r":
			// Retry the currently selected failed page.
		...
		case "R", "a":
			// Retry ALL failed pages at once.
			if m.retryCh == nil {
				return m, nil
			}
			var slugs []string
			for i := range m.pages {
				if m.pages[i].Status == StatusFailed {
					slugs = append(slugs, m.pages[i].Slug)
					m.pages[i].Status = StatusRetrying
					m.pages[i].AgentStatus = "[waiting to retry]"
					m.pages[i].Err = ""
				}
			...
			ch := m.retryCh
			return m, func() tea.Msg {
				// Cmd runs in its own goroutine; block until runner drains the queue.
...
func (m *Model) renderPages(sb *strings.Builder) {
	...
	for _, p := range m.pages {
		if p.Status == StatusRunning || p.Status == StatusRetrying {
			running++
		}
	}
	...
	if fail > 0 || running > 0 {
		header = fmt.Sprintf("Phase 2 — Generate Pages (%d/%d)  done:%d  fail:%d  run:%d",
			counted, total, done, fail, running)
	}
	...
	// Show error for the currently selected failed page.
	if m.cursor >= 0 && m.cursor < len(m.pages) {
		sel := m.pages[m.cursor]
		if sel.Status == StatusFailed && strings.TrimSpace(sel.Err) != "" {
			errLine := abbrevVis(strings.ReplaceAll(sel.Err, "\n", " "), 90)
			sb.WriteString(styleSFailed.Render("  error: " + errLine))
			sb.WriteByte('\n')
		}
	}
	...
	// Dynamic footer
	if fail > 0 {
		sb.WriteString(styleDim.Render("↑/↓: navigate | r: retry one | R/a: retry all failed | s: skip & commit | ctrl+c: quit"))
	} else {
...
func (m *Model) renderDone(sb *strings.Builder) {
	...
	sb.WriteString(styleDim.Render("  Run wikify browse to view your docs"))
	sb.WriteByte('\n')
	sb.WriteByte('\n')
	sb.WriteString(styleDim.Render("ctrl+c: quit"))
	sb.WriteByte('\n')
...
func (m *Model) recount() {
	m.doneCount, m.failCount = m.countStatuses()
}

func (m Model) countStatuses() (done, fail int) {
	for _, p := range m.pages {
		switch p.Status {
		case StatusDone:
			done++
